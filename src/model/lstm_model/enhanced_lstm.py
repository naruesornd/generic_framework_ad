...
def _plot_hybrid(timestamps, y_true, y_phys, y_resid, y_total, anomalies,
    ...
    """
    Interactive 4-panel Plotly figure with timestamps on x-axis.
      1. Raw sensor vs total prediction + anomaly markers
      2. Physics vs ML residual (dual y-axis)
      3. Absolute error with 3σ threshold line
      4. Anomaly flag timeline
    """
    errors = np.abs(y_true - y_total)
    anom_idx = np.where(anomalies)[0]
    buffer_intervals = _mask_to_intervals(buffer_mask, timestamps)

    # 4 stacked panels with shared x-axis. Panel 2 doesn't use a secondary
    # y-axis here (we work around it by overlaying a second trace via add_trace
    # with `yaxis='y3'` style — make_subplots needs `secondary_y` per row).
    fig = make_subplots(
        rows=4, cols=1,
        shared_xaxes=True,
        ...
        subplot_titles=(
            "True sensor vs hybrid prediction",
            "Physics contribution vs ML residual (dual axis)",
            "Prediction error and 3σ threshold",
            "Anomaly flags",
        ),
        specs=[
            [{"secondary_y": False}],
    ...
    # ── Panel 2: Physics + ML residual on dual axes ──────────────────────────
    fig.add_trace(go.Scatter(
    ...
    fig.add_trace(go.Scatter(
        ...
        fillcolor="rgba(220,40,40,0.45)",
        showlegend=False,
        hovertemplate="<b>%{x}</b><br>flag=%{y}<extra></extra>",
    ), row=4, col=1)

    # Buffer zone shading — applied to panels 1, 3, 4 (panel 2 is busy enough)
    for row in (1, 3, 4):
        _add_buffer_shading(
            fig, buffer_intervals, row=row, col=1,
            label=f"Buffer (±{cycle_buffer})" if row == 1 else None,
        )
    ...
    fig.update_yaxes(title_text="Physics value", title_font=dict(color="darkorange"),
                     tickfont=dict(color="darkorange"),
                     row=2, col=1, secondary_y=False)
    fig.update_yaxes(title_text="ML residual",   title_font=dict(color="purple"),
    ...
    fig.update_xaxes(title_text="Timestamp", row=4, col=1, rangeslider_visible=show_rangeslider)

    # Save HTML for offline viewing
    os.makedirs(f"{save_dir}/../img", exist_ok=True)
    out_html = f"{save_dir}/../img/hybrid_{feature_name}.html"
